...
function Home() {
  const [products, setProducts] = useState([]); const [error, setError] = useState(''); const [loading, setLoading] = useState(true);
  ...
  return <main><section className="shop-heading"><div><p className="eyebrow">Marketplace</p><h1>Find something worth keeping.</h1><p className="muted">Browse products from independent sellers.</p></div><span className="catalog-count">{products.length} products</span></section>
    {loading ? <Loading /> : error ? <ErrorMessage message={error} /> : products.length === 0 ? <p className="state">No products are available yet.</p> : <div className="product-grid">{products.map((product) => <article className="product-card" key={product.product_id}><Link to={`/products/${product.product_id}`}><ProductImage product={product} /></Link><div className="product-card-body"><p className="seller-label">Available now</p><h2>{product.product_name}</h2><p className="truncate">{product.product_description}</p><div className="product-meta"><strong>{money(product.price)}</strong><span>{product.available_stock} in stock</span></div><Link className="button secondary-button" to={`/products/${product.product_id}`}>View details</Link></div></article>)}</div>}
  </main>;
}
...
function Admin() { return <Guard role="admin"><main><section className="dashboard-hero admin-hero"><p className="eyebrow">Admin dashboard</p><h1>Marketplace operations.</h1><p className="muted">Your backend currently exposes order completion as the admin action.</p></section><div className="dashboard-grid"><section className="dashboard-panel"><p className="eyebrow">Available now</p><h2>Order completion</h2><p className="muted">Order management endpoints can be connected here as they are added to the backend.</p><span className="status-pill">Awaiting order API views</span></section><section className="dashboard-panel"><p className="eyebrow">System status</p><h2>Authentication active</h2><p className="muted">This control room is protected by the admin session.</p><span className="status-pill green">Connected</span></section></div></main></Guard>; }
