...
function Home() {
  ...
  useEffect(() => {
    ...
    const load = selectedCategory === 'all' ? api.products(query) : api.productsByCategory(selectedCategory, query);
  ...
  return <main><section className="shop-heading"><div><p className="eyebrow">Marketplace</p><h1>Find something worth keeping.</h1><p className="muted">Browse products from independent sellers.</p></div><span className="catalog-count">{products.length} products</span></section>
    <form className="shop-search" role="search" onSubmit={(event) => { event.preventDefault(); setSearch(searchInput); }}><label htmlFor="product-search">Search the marketplace</label><div><input id="product-search" type="search" value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search products by name or description" /><button type="button" className="small-button" onClick={() => { setSearchInput(''); setSearch(''); }} disabled={!searchInput && !search}>Clear</button></div></form>
